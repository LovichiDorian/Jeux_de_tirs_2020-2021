		...
		public function LocalizedTextField() 
		{
			...
			for (var i:int = 0; i < numChildren; i++) {
				if (getChildAt(i) is TextField) {
					...
					if (lText.multiline && lContent.charAt(lContent.length - 1) == "\r") 
                        lContent=lContent.substr(0, lContent.length - 1);
					
					if (defaultLanguage) 
                        lText.text = lContent;
					else{
                        var lTranslation:XMLList = texts.file.body["trans-unit"].(source == lContent);
                        
                        if (!lTranslation.hasOwnProperty("target"))
                            throw new Error("No translation found for \"" + lContent + "\"");
                        
						lText.text = lTranslation.target;
                    }
				}
			}
